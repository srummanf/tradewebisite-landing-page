...
interface CardProps {
    topRightGradient: string; // URL for the top-right gradient image
    bottomLeftGradient: string; // URL for the bottom-left gradient image
...
const Card: React.FC<CardProps> = ({
    ...
    return (
        <div className="relative w-full max-w-6xl rounded-3xl overflow-hidden bg-gradient-to-br from-navy-900 via-slate-900 to-cyan-900 p-8 md:p-12 border border-slate-500">
            ...
            <div
                className="absolute bottom-0 left-0 w-[1700px] h-[1700px] bg-contain bg-no-repeat bg-bottom-left pointer-events-none"
                style={{
                    backgroundImage: `url('${bottomLeftGradient}')`
                }}
            />
            <div
                className="absolute top-0 right-0 w-[1700px] h-[1700px] bg-contain bg-no-repeat bg-top-right pointer-events-none"
                style={{
                    backgroundImage: `url('${topRightGradient}')`
                }}
            />
