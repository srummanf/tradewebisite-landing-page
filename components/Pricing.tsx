  ...
  return (
    <div className="relative isolate bg-black px-6 py-24 sm:py-32 lg:px-8">
      ...
      <div className="mx-auto max-w-4xl text-center">
        <h2 className="text-base/7 font-semibold text-indigo-300">Pricing</h2>
        <p className="mt-2 text-white text-5xl font-semibold tracking-tight sm:text-6xl">
      ...
      <div className="mx-auto mt-16 grid max-w-lg grid-cols-1 items-center gap-y-6 sm:mt-20 sm:gap-y-0 lg:max-w-4xl lg:grid-cols-2">
        {tiers.map((tier, tierIdx) => (
          ...
          >
            <h3
              id={tier.id}
              className={classNames(tier.featured ? "text-indigo-400" : "text-indigo-300", "text-base/7 font-semibold")}
            >
              {tier.name}
            </h3>
              ...
              {tier.features.map((feature) => (
                <li key={feature} className="flex gap-x-3">
                  <CheckIcon
                    aria-hidden="true"
                    className={classNames(
                      tier.featured ? "text-indigo-400" : "text-indigo-300",
                      "h-6 w-5 flex-none"
                    )}
                  />
                  {feature}
